...
exports.validate=[body('category_id').isInt(),body('slug').optional({checkFalsy:true}).trim().isLength({max:160}),body('name_ar').trim().isLength({min:2,max:220}),body('name_en').trim().isLength({min:2,max:220}),body('price').isFloat({min:0}),body('stock').isInt({min:0}),body('condition_status').optional().isIn(['new','used'])];
